...
class Get_allMakes(generics.ListCreateAPIView):
    ...
    print "car_make"
    queryset = Car_Master_Data.objects.order_by().values('make_name').distinct()
    # print queryset
    serializer_class = MaterCarSerializer
...
def Get_DataFromCity(request,city):
    ...
    try:
        print "cities"
        city_data = CarDetails.objects.order_by("price").filter(city="Delhi")
        print city_data
